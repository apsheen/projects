...
void shutdown(void);
bool startup(void);
bool check_gy(int ch);
bool check_gx(int ch);
bool check_block(int ch);
bool won(int ch);
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < 9; i++)
        for(int j = 0; j < 9; j++)
            g.board_1[i][j] = g.board[i][j];
            
    draw_numbers();
    show_cursor();
    
    //for undoing
    //stores the last number inputted
    int last_num = 0;
    //stores the x and y coordinates of where the last number was inputted
    int cursorY = 0;
    int cursorX = 0;
    //number inputted this time
    int buffer_num = 0;
    ...
    do
    {
        ...
        switch(ch)
        {
            ...
            case '9':
                if(won(ch - '0') == true)
                {
                    show_banner("Congrats! You've solved the sudoku!");
                    show_cursor();
                    draw_numbers();
                    break;
                }
                ...
                if(check_gy(ch - '0') == false)
                {
                    show_banner("Cannot have the same number in a row!");
                    show_cursor();
                }
                    
                if(check_gx(ch - '0') == false)
                {
                    show_banner("Cannot have the same number in a column!");
                    show_cursor();
                }
                
                if(check_block(ch - '0') == false)
                {
                    show_banner("Cannot have the same number in the same block!");
                    show_cursor();
                }
            ...
            case KEY_DC:
                ...
                last_num = g.board[g.y][g.x];
                cursorY = g.y;
                cursorX = g.x;
                buffer_num = 0;
                                
                if(g.board_1[g.y][g.x] == 0)
                {
                    hide_banner();
                    g.board[g.y][g.x] = 0;
                    draw_numbers();
                ...
                {
                    show_banner("Gotcha! Can't change numbers given at the start!");
                    show_cursor();
                }
                ...
                case 'U':
                case CTRL('Z'):
                    if(g.board_1[cursorY][cursorX] == 0)
                        {
                            g.board[cursorY][cursorX] = last_num;
                            draw_numbers();
                            show_cursor();
                            hide_banner();
                            show_cursor();
...
void draw_numbers()
{
    ...
    for (int i = 0; i < 9; i++)
    {
        for (int j = 0; j < 9; j++)
        {
            if((has_colors()) && (won(ch - '0') != true))
            {
                //original numbers color turn on
                if((g.board[i][j] == g.board_1[i][j]) && (g.board_1[i][j] != 0))
                    attron(COLOR_PAIR(PAIR_LOGO));
                
                //user imputted numbers color turn on
                else
...
/*
 * Shows banner. Must be called after show_grid has been called at least once.
 */
void show_banner(char *b)
{
    // enable color if possible
...
//returns true if there are no repeats in a row, else returns false
bool check_gy(int ch)
{
    if(ch == 0)
    {
...
//returns true if there are no repeats in a column, else returns false
bool check_gx(int ch)
{
    if(ch == 0)
    {
...
//returns true if there are no repeats in a block, else returns false
bool check_block(int ch)
{
    if(ch == 0)
        return true;
    ...
    for(int i = 0; i < 3; i++)
    {
        for(int j = 0; j < 3; j++)
        {
            if(g.board[(blocky + i)][(blockx + j)] == ch)
                counter++;
        }
    }
...
//returns true if there are no blanks and no repeats in a row, column, or block, else returns false
bool won(int ch)
{
    for(int i = 0; i < 9; i++)
    {
    ...
    if((check_block(ch - '0') == true) && (check_gy(ch - '0') == true) && (check_gx(ch - '0') == true))
        return true;
