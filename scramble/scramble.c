 ...
 *
 * Usage: scramble [#], where # is an optional grid number.
 ***************************************************************************/
...
// defines word as having array of letters plus flag indicating whether word has been found on grid
typedef struct
{
    bool found;
...
// defines dictionary as having size, an array of words
struct
{
    int size;
    word words[WORDS];
...
/**
 * Looks up word s in dictionary. If found (for 1st time), flags word as found so user can't score with it again, returns true
 */
bool lookup(string s)
{
    for(int i = 0; i < (172806); i++)
    {
...
/**
 * Scrambles grid by rotating it 90 deg. clockwise, where grid[0][0] rotates to grid[0][DIMENSION - 1].
 */
void scramble(void)
{
    char grid90[DIMENSION][DIMENSION];
    
    for(int i = 0; i < 4; i++)
    {
        for(int j = 0; j < 4; j++)
        {
            grid90[j][(3 - i)] = grid[i][j];
        }
    }
